...
OPENAI_LARGE_MODEL = os.getenv("OPENAI_LARGE_MODEL", os.getenv("OPENAI_MODEL", "gpt-4o-mini"))
OPENAI_SMALL_MODEL = os.getenv("OPENAI_SMALL_MODEL", OPENAI_LARGE_MODEL)
OPENAI_TIMEOUT = int(os.getenv("OPENAI_TIMEOUT", "30"))
PRIMARY_LLM_TRACK_LIMIT = int(os.getenv("PRIMARY_LLM_TRACK_LIMIT", "5"))
RETRY_LLM_TRACK_LIMIT = int(os.getenv("RETRY_LLM_TRACK_LIMIT", "5"))
...
def call_openai_chat(
    ...
    except Exception as exc:  # pragma: no cover - 응답 파싱 실패 대비
        LOGGER.warning("OpenAI 응답 파싱 실패: %s", exc)
        return None


def parse_llm_tracks(raw_text: Optional[str], limit: int) -> List[LLMTrackSuggestion]:
    if not raw_text:
        return []
    json_payload = _extract_json_block(raw_text)
    if json_payload is None:
        LOGGER.warning("LLM 응답에서 JSON을 찾지 못했습니다: %s", raw_text)
        return []
    try:
    ...
    results: List[LLMTrackSuggestion] = []
    for item in tracks:
        title = (item.get("title") or item.get("name") or "").strip()
        artist = (item.get("artist") or item.get("artists") or "").strip()
        if not title or not artist:
            continue
        reason_raw = (item.get("reason") or item.get("notes") or "").strip()
        tempo_hint = item.get("tempo")
        try:
            tempo_hint = float(tempo_hint) if tempo_hint is not None else None
...
def generate_llm_track_suggestions(state: str, bpm_target: Optional[int], limit: int) -> List[LLMTrackSuggestion]:
    if not has_openai_support():
        return []
    system_prompt = (
        "당신은 생체 신호 기반 음악 추천 전문가입니다."
        " 상태와 목표 BPM에 맞는 곡을 제안하고 반드시 JSON 객체를 반환하세요."
        " tracks 배열의 각 항목에 title, artist, tempo(가능하면 숫자), reason을 포함하십시오."
    )
    info = [f"state: {state}"]
    if bpm_target is not None:
        info.append(f"bpm_target: {bpm_target}")
    info.append(f"limit: {limit}")
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": "\n".join(info)},
    ]
    raw = call_openai_chat(OPENAI_LARGE_MODEL, messages, temperature=0.3)
    suggestions = parse_llm_tracks(raw, limit)
    if suggestions:
        LOGGER.info("LLM이 %d개의 후보를 제안했습니다.", len(suggestions))
    return suggestions


def search_spotify_for_suggestions(
    token: str,
    suggestions: List[LLMTrackSuggestion],
    source_label: str,
) -> List[TrackCandidate]:
    if not suggestions:
        return []
    ...
    for suggestion in suggestions:
        query = f"track:{suggestion.title} artist:{suggestion.artist}"
        ...
        if response.status_code != 200:
            LOGGER.warning("Spotify 검색 실패(%s): %s", query, response.text)
            continue
        items = (response.json().get("tracks") or {}).get("items", [])
        if not items:
            LOGGER.info("Spotify 검색 결과가 없습니다: %s", query)
            continue
        item = items[0]
        uri = item.get("uri")
        ...
        if not uri or not track_id:
            continue
        name = item.get("name", suggestion.title)
    ...
    tempos = fetch_audio_features(token, [candidate.track_id for candidate in found])
    for candidate in found:
        candidate.tempo = tempos.get(candidate.track_id)
    return found


def deduplicate_candidates(candidates: List[TrackCandidate]) -> List[TrackCandidate]:
    seen: set[str] = set()
    unique: List[TrackCandidate] = []
    for candidate in candidates:
...
def collect_candidates(
    ...
    cache_key = (state.lower(), bpm_target if bpm_target is not None else -1)
    ...
    aggregated: List[TrackCandidate] = []
    if has_openai_support():
        suggestions = generate_llm_track_suggestions(state, bpm_target, PRIMARY_LLM_TRACK_LIMIT)
        aggregated.extend(
            search_spotify_for_suggestions(token, suggestions, "llm_primary")
        )

    aggregated.extend(fetch_recommendations(token, params))
    aggregated = deduplicate_candidates(aggregated)
...
def retry_candidates_with_small_llm(
    token: str,
    state: str,
    bpm_target: Optional[int],
    attempted: List[TrackCandidate],
) -> List[TrackCandidate]:
    if not has_openai_support():
        return []
    failed_desc = [f"{cand.name} - {', '.join(cand.artists)}" for cand in attempted]
    system_prompt = (
        "이전 후보들이 재생되지 않았습니다."
        " 유사한 무드와 BPM을 유지하면서 다른 곡을 JSON tracks 배열로 제안하세요."
    )
    info = [f"state: {state}"]
    if bpm_target is not None:
        info.append(f"bpm_target: {bpm_target}")
    if failed_desc:
        info.append("failed_candidates: " + "; ".join(failed_desc))
    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": "\n".join(info)},
    ]
    raw = call_openai_chat(OPENAI_SMALL_MODEL, messages, temperature=0.4, max_tokens=600)
    suggestions = parse_llm_tracks(raw, RETRY_LLM_TRACK_LIMIT)
    if not suggestions:
        return []
    LOGGER.info("소형 LLM이 재시도 후보 %d개를 반환했습니다.", len(suggestions))
    return search_spotify_for_suggestions(token, suggestions, "llm_retry")
...
def set_target(request: PlaybackRequest) -> PlaybackResponse:
    ...
    candidates = collect_candidates(token, request.state, request.bpm_target, params)
    if not candidates:
        logger.warning("Spotify/LLM 후보를 찾지 못해 로컬 폴백을 진행합니다")
        return handle_local_playback(request, source="no_spotify_candidates")
    ...
    if selected is None:
        retry_candidates = retry_candidates_with_small_llm(
            token, request.state, request.bpm_target, attempted
        )
        if retry_candidates:
            retry_candidates = deduplicate_candidates(retry_candidates)
            retry_candidates = apply_cooldown_filter(retry_candidates) or retry_candidates
    ...
    record_cooldown(selected)
    notes = f"재생 곡: {selected.name} / 아티스트: {', '.join(selected.artists)}"
    log_playback_event("spotify", request.state, request.bpm_target, expected_device, selected.uri, final_source, notes)
    logger.info("Spotify 재생 성공: %s", notes)
